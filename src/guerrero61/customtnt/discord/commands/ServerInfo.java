    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            onlinePlayer.append(Formatter.FText(Formatter.FText("%essentials_afk%", true, player).equals("yes") ?
                    Formatter.FText("[AFK] ", true, player) : "", true, player)).append(player.getName()).append("\n");
        }
        ...
        EmbedBuilder embed = new EmbedBuilder().setAuthor("Información del server | TnTServer", null,
                autorAvatar).setThumbnail(api.getSelfUser().getAvatarUrl()).addField("Versión",
                Bukkit.getVersion().replace("git-", "") + "\n" +
                (Bukkit.getOnlineMode() ? "Premium" : "Premium/No Premium"), true).addField(Main.getPlayerCount(),
                (Bukkit.getOnlinePlayers().size() == 0) ? "No hay nadie jugando" : onlinePlayer.toString(),
                true).addBlankField(true).addField("Jugadores unicos",
                Integer.toString(Bukkit.getOfflinePlayers().length), true).addField("TPS", StringTPS,
                true).addBlankField(true).addField("Horas de tormenta",
                (Objects.requireNonNull(Bukkit.getWorld("world")).hasStorm() ? StormActionBar.stormTime :
                         "No hay tormenta"), true).setColor(new Color(255, 61, 61)).setFooter(Main.getIp(),
